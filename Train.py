...
data = {
    'train':
    datasets.ImageFolder(root='./dataset/train', transform=image_transforms['train']),
    'test':
    datasets.ImageFolder(root='./dataset/test', transform=image_transforms['test']),
}
...
model = train(model, optimizer,loss,dataloaders['train'],dataloaders['test'],20)
